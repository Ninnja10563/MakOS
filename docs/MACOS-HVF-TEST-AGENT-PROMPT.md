...
3. Confirm no QEMU is running. Run these gates in order and save complete logs:
   ...
   headers, depth two, recursive absolute-quoted resolution, four macros,
   ...
   plus
   `malformed_preprocessor=define,endif,unterminated,duplicate-else,expression,elif-after-else,zero-divisor,shift-range,overflow,conditional-syntax,conditional-selected-trap-denied`.
