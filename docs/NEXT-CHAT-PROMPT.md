...
runtime. The guest-native AArch64 toolchain now reads a valid multi-statement C
  ...
  arithmetic, register locals, equality control flow, a 64-byte non-leaf frame,
  and a cross-object call into A64, and emits/persists/reopens three genuine
  ...
  unresolved-symbol and duplicate-definition inputs, emits `ET_EXEC`, executes
  both linked C paths, and runs the final ELF twice with status 42 in focused
  Pi/QEMU TCG runtime.
...
One visible Pi/QEMU TCG login milestone is running at handoff: PID 351586, VNC
`127.0.0.1:5901`, session `build/makos-pi-visible-selfhost-three-object-hardened-dMPn4hSC`, private
boot/data/vars in that session, and QMP
`build/makos-pi-visible-selfhost-three-object-hardened-dMPn4hSC/qmp.sock`. Stop it through QMP before
