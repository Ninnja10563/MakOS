...
static void run_selfhost_probe(void) {
    ...
    if (status == 42)
        write_text(
            "MAKOS_AARCH64_SELFHOST_LINK_OK source=guest-makfs sources=4 "
            "languages=aarch64-asm,c-subset-v1 compiler=guest-native "
            "assembler=guest-native linker=guest-native objects=4 "
            "object_format=elf64-et-rel relocations=R_AARCH64_CALL26:3 "
            "symbols=_start,answer,adjust,combine,helper "
            "build_manifest=/home/user/generated.build "
            "build_driver=makbuild-v1 build_inputs=4 cache=makstate-v2 "
            "cache_hits=0 cache_misses=4 state_committed=1 "
            ...
            "c_operators=mul,sdiv,srem,neg,sub,add "
            "signed_division_results=20:6,-20:-6 "
            "signed_remainder_results=20:2,-20:-2 "
            "unary_negation_results=42:-42,-42:42 "
            "arithmetic_object=elf64-et-rel:784 "
            "c_relations=eq,ne,lt,le,gt,ge "
            "branch_results=42,86 loop_results=42,2 memory_results=42,2 "
            "pointer_call=answer-to-adjust pointee_results=42,44,2 "
            "delta_results=1:42,2:44,1:2 "
            "array_results=41:42:0,42:0:44,1:2:0 pointer_offset_call=1 "
            "pointer_variable_offset=delta dynamic_pointer_adds=2 "
            "signed_pointer_offset=-1:42 signed_pointer_difference=3:-3 "
            ...
            "malformed_c_denied=18 malformed_relocation_denied=1 "
...
static void run_native_smp_probe(void) {
    uint64_t pid = syscall4(SYS_PROCESS_SPAWN, 18, 0, 0, 0);
    if (pid == UINT64_MAX) {
        write_text("native-smp: launch failed\n");
        return;
    }
    uint64_t status;
    while ((status = syscall4(SYS_PROCESS_WAIT, pid, 0, 0, 0)) == UINT64_MAX)
        syscall4(SYS_YIELD, 0, 0, 0, 0);
    if (status == 42)
        write_text("MAKOS_AARCH64_NATIVE_SMP_REAP_OK fixture=upstream-musl-pthread role=native status=42 lifecycle=spawn,threads,ap-dispatch,block,wake,join,exit,wait,reap\n");
    else
        write_text("native-smp: runtime failed\n");
}

static void run_python_smp_probe(void) {
    uint64_t pid = syscall4(SYS_PROCESS_SPAWN, 19, 0, 0, 0);
    if (pid == UINT64_MAX) {
        write_text("python-smp: launch failed\n");
        return;
    }
    uint64_t status;
    while ((status = syscall4(SYS_PROCESS_WAIT, pid, 0, 0, 0)) == UINT64_MAX)
        syscall4(SYS_YIELD, 0, 0, 0, 0);
    if (status == 42)
        write_text("MAKOS_AARCH64_PYTHON_SMP_REAP_OK fixture=upstream-musl-pthread role=python status=42 lifecycle=spawn,threads,ap-dispatch,block,wake,join,exit,wait,reap\n");
    else
        write_text("python-smp: runtime failed\n");
}
    ...
    for (;;) {
        ...
        while (!signed_out) {
            ...
            if (key == '\n') {
                ...
                } else if (exact(command, command_length, "native-smp")) {
                    run_native_smp_probe();
                } else if (exact(command, command_length, "python-smp")) {
                    run_python_smp_probe();
