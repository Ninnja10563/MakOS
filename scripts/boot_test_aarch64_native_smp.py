...
READY_MARKER = (
    ...
    b"toolchain-leaders-least-loaded-ap roles=firefox,native,python,toolchain "
...
PYTHON_PROCESS_MARKER = b"MAKOS_AARCH64_PYTHON_SMP_PROCESS_OK"
PYTHON_OVERLAP_MARKER = b"MAKOS_AARCH64_PYTHON_SMP_OVERLAP_OK"
PYTHON_PTHREAD_MARKER = (
    b"MAKOS_PYTHON_SMP_PTHREAD_OVERLAP_OK workers=3 rendezvous=ready "
    b"release=bounded affinity=default:0xe,explicit singleton=0x2,0x4,0x8 "
    b"restored=0xe get=kernel-owned placement=least-reserved-ap "
    b"migrations=automatic:load,forced:3 caller_selected_automatic=0"
)
PYTHON_MIGRATION_MARKER = b"MAKOS_AARCH64_APPLICATION_MIGRATION_OK role=python"
PYTHON_RESULT_MARKER = b"MAKOS_AARCH64_PYTHON_SMP_OK"
PYTHON_REAP_MARKER = (
    b"MAKOS_AARCH64_PYTHON_SMP_REAP_OK fixture=upstream-musl-pthread "
    b"role=python status=42"
)


def validate_python_role(
    decoded: str,
) -> tuple[tuple[int, int, int], tuple[int, int, int], int]:
    processes = re.findall(r"MAKOS_AARCH64_PYTHON_SMP_PROCESS_OK pid=(\d+)", decoded)
    overlaps = re.findall(
        r"MAKOS_AARCH64_PYTHON_SMP_OVERLAP_OK group_pid=(\d+) "
        r"cpu_mask=(0x[0-9a-f]+) tids=(\d+),(\d+),(\d+)",
        decoded,
    )
    results = re.findall(
        r"MAKOS_AARCH64_PYTHON_SMP_OK cpu_mask=(0x[0-9a-f]+) "
        r"dispatches=(\d+),(\d+),(\d+) overlap_mask=(0x[0-9a-f]+) "
        r"overlap_tids=(\d+),(\d+),(\d+) worker_role=python "
    ...
    if not processes or not overlaps or not results:
        raise AssertionError("Python-role production-SMP fields were malformed")
    group, marker_mask, mt1, mt2, mt3 = overlaps[-1]
    (
        mask_text, d1, d2, d3, overlap_text, t1, t2, t3,
        p1, p2, p3, placement_text, migrations_text,
        source_text, target_text, delta_text, drops_text,
    ) = results[-1]
    if group != processes[-1]:
        raise AssertionError("Python-role overlap group did not match launch")
    dispatches = (int(d1), int(d2), int(d3))
    placements = (int(p1), int(p2), int(p3))
    ...
    migrations = int(migrations_text)
    if int(mask_text, 16) & 0xE != 0xE or any(value == 0 for value in dispatches):
        raise AssertionError(
            f"Python-role workers did not use every AP: dispatches={dispatches}"
        )
    if (
        int(placement_text, 16) != 0xE
        or any(value == 0 for value in placements)
        or migrations < 1
        or int(source_text, 16) & 0xE == 0
        or int(target_text, 16) & 0xE == 0
        or int(delta_text) != 64
        or int(drops_text) != 0
    ):
        raise AssertionError(
            "Python-role automatic balancing evidence was incomplete: "
            f"placements={placements} migrations={migrations}"
        )
    if marker_overlap_mask != overlap_mask or marker_overlap_tids != overlap_tids:
        raise AssertionError("Python-role live/final overlap evidence disagreed")
    active_tids = [
        overlap_tids[cpu - 1]
    ...
    if (
        ...
        raise AssertionError(
            f"Python-role workers did not overlap: mask={overlap_mask:#x} "
            f"tids={overlap_tids}"
        )
    return dispatches, placements, migrations
...
def main() -> int:
    ...
    python_dispatches = (0, 0, 0)
    python_placements = (0, 0, 0)
    python_migrations = 0
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            with qmp_parent:
                ...
                common.send_command(stream, "python-smp")
                common.wait_for_output(
                    selector, process, output, PYTHON_PROCESS_MARKER, 30
                )
                common.wait_for_output(
                    selector, process, output, PYTHON_OVERLAP_MARKER, 60
                )
                common.wait_for_output(
                    selector, process, output, PYTHON_PTHREAD_MARKER, 30
                )
                common.wait_for_output(
                    selector, process, output, PYTHON_MIGRATION_MARKER, 30
                )
                common.wait_for_output(
                    selector, process, output, PYTHON_RESULT_MARKER, 90
                )
                common.wait_for_output(
                    selector, process, output, PYTHON_REAP_MARKER, 20
                )

                decoded = output.decode(errors="replace")
                python_dispatches, python_placements, python_migrations = (
    ...
    print(
        ...
        f"builtin_role=python python_dispatches={python_dispatches[0]},{python_dispatches[1]},{python_dispatches[2]} "
        f"python_placements={python_placements[0]},{python_placements[1]},{python_placements[2]} "
        f"python_migrations={python_migrations} python_status=42"
    )
    return 0
