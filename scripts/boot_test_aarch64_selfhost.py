...
LINKER_MARKER = (
    ...
    b"linked_bytes=500 output_bytes=815 helper_result=42 persisted_reopened=1 manifest_input_bounds=2..6 malformed_build_denied=6 malformed_c_denied=18 "
...
HEADER_DEP_MARKER = (
    ...
    b"if_expression=defined,numeric,comparison,not,and,or elif=selected "
    b"include_guard=deduplicated fingerprint=expanded-source"
)
HEADER_GUARD_MARKER = (
    b"MAKOS_AARCH64_C_PREPROCESSOR_GUARD_OK headers=2 max_depth=2 macros=4 "
    ...
    b"elif-after-else-denied depth_limit=4"
)
...
def main() -> int:
    ...
    print(
        ...
        "header_dependency=quoted-absolute-recursive headers=2 max_depth=2 depth_limit=4 preprocessor=object-macro-if-expressions macros=4 conditional_depth=2 if_expression=defined,numeric,comparison,not,and,or elif=selected include_guard=deduplicated fingerprint=expanded-source malformed_headers=missing,relative,cycle,overdepth-denied malformed_preprocessor=define,endif,unterminated,duplicate-else,expression,elif-after-else-denied transitive_header_execution=42 "
        ...
        "linked_bytes=500 output_bytes=815 helper_result=42 persisted_reopened=1 manifest_input_bounds=2..6 malformed_build_denied=6 malformed_c_denied=18 "
