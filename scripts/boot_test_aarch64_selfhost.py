...
LINKER_MARKER = (
    ...
    b"c_source=/home/user/generated-program.c translation_unit_functions=2 "
    b"c_abi=aapcs64-int32-pointer64 "
    b"c_features=multi-function,multi-parameter,parameter,pointer-parameter,local,array,array-decay,index,assignment,pointer,pointer-add,address-of,address-expression,dereference,if,equality,inequality,while,call,return "
    b"max_parameters=2 max_call_arguments=2 nonleaf_frame=96 c_operators=mul,sub,add branch_results=42,86 "
    b"loop_results=42,2 memory_results=42,2 pointer_call=answer-to-adjust "
    b"pointee_results=42,44,2 delta_results=1:42,2:44,1:2 array_results=41:42,42:44,1:2 pointer_offset_call=1 "
    b"code_bytes=76,140,140 object_bytes=688,888 intra_object_call=1 "
    b"linked_bytes=356 output_bytes=815 persisted_reopened=1 malformed_c_denied=14 "
...
EXECUTION_MARKER = (
    ...
    b"symbols=_start,answer,adjust c_source=/home/user/generated-program.c "
    b"translation_unit_functions=2 c_abi=aapcs64-int32-pointer64 "
    b"c_features=multi-function,multi-parameter,parameter,pointer-parameter,local,array,array-decay,index,assignment,pointer,pointer-add,address-of,address-expression,dereference,if,equality,inequality,while,call,return "
    b"max_parameters=2 max_call_arguments=2 nonleaf_frame=96 c_operators=mul,sub,add branch_results=42,86 "
    b"loop_results=42,2 memory_results=42,2 pointer_call=answer-to-adjust "
    b"pointee_results=42,44,2 delta_results=1:42,2:44,1:2 array_results=41:42,42:44,1:2 pointer_offset_call=1 "
...
def main() -> int:
    ...
    print(
        ...
        "symbols=_start,answer,adjust translation_unit_functions=2 "
        "c_abi=aapcs64-int32-pointer64 "
        "c_features=multi-function,multi-parameter,parameter,pointer-parameter,local,array,array-decay,index,assignment,pointer,pointer-add,address-of,address-expression,dereference,if,equality,inequality,while,call,return "
        "max_parameters=2 max_call_arguments=2 nonleaf_frame=96 c_operators=mul,sub,add branch_results=42,86 "
        "loop_results=42,2 memory_results=42,2 pointer_call=answer-to-adjust "
        "pointee_results=42,44,2 delta_results=1:42,2:44,1:2 array_results=41:42,42:44,1:2 pointer_offset_call=1 "
        "code_bytes=76,140,140 object_bytes=688,888 intra_object_call=1 "
        "linked_bytes=356 output_bytes=815 persisted_reopened=1 malformed_c_denied=14 "
