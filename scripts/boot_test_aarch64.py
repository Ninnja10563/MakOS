    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            with client:
                ...
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_MAKBUILD_OK mode=fixture manifest=/home/user/generated.build startup=sysv argc=2 envc=1 seeded=1 cache=makstate-v2 build_inputs=4 cache_hits=0 cache_misses=4 state_committed=1 status=42",
                )
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_C_SIX_FUNCTION_OK functions=6 calls=5 relocations=R_AARCH64_CALL26:5 object=elf64-et-rel linked=1 result=42 max_functions=6 overflow=7-denied",
                )
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_C_SIX_ARGUMENT_OK parameters=6 call_arguments=6 registers=x0-x5 callee_saved=x23-x28 frame=112 object=elf64-et-rel:808 relocation=R_AARCH64_CALL26 direct_result=42 same_object_call_result=42 overflow=7-denied",
                )
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_C_BRANCH_BLOCK_OK forms=if,if-else,nested-if,nested-loop body=bounded-control-assignment continuation=return max_depth=4 object=elf64-et-rel symbols=choose,bump,nested,accumulate linked=1 wx=denied results=42,2,5,8,42,2,1,6 malformed=empty-else,branch-declaration-denied,depth-5-denied",
                )
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_LINKER_OK sources=4 languages=aarch64-asm,c-subset-v1 compiler=guest-native assembler=guest-native objects=4 format=elf64-et-rel linker=guest-native relocations=R_AARCH64_CALL26:3 symbols=_start,answer,adjust,combine,helper output=/home/user/generated-aarch64.elf build_manifest=argv1 build_driver=makbuild-v1 build_inputs=4 cache=makstate-v2 cache_hits=0 cache_misses=4 state_committed=1 c_sources=/home/user/generated-program.c,/home/user/generated-library.c,/home/user/generated-helper.c translation_unit_functions=2,1,1 c_abi=aapcs64-int32-pointer64 c_features=multi-function,multi-parameter,six-argument,signed-arithmetic,parameter,pointer-parameter,local,array,array-decay,index,assignment,pointer,pointer-add,pointer-variable-add,pointer-difference,address-of,address-expression,dereference,if,if-assignment,if-else,nested-control,equality,inequality,relational,while,call,return max_parameters=6 max_call_arguments=6 nonleaf_frame=96,112 three_argument_result=42 three_argument_link=et-rel,same-object six_argument_result=42 six_argument_link=et-rel,same-object c_operators=mul,sdiv,srem,neg,sub,add signed_division_results=20:6,-20:-6 signed_remainder_results=20:2,-20:-2 unary_negation_results=42:-42,-42:42 arithmetic_object=elf64-et-rel:784 c_relations=eq,ne,lt,le,gt,ge branch_results=42,86 loop_results=42,2 memory_results=42,2 pointer_call=answer-to-adjust pointee_results=42,44,2 delta_results=1:42,2:44,1:2 array_results=41:42:0,42:0:44,1:2:0 pointer_offset_call=1 pointer_variable_offset=delta dynamic_pointer_adds=2 signed_pointer_offset=-1:42 signed_pointer_difference=3:-3 relational_results=gt:42:0,le:42:0,ge:42:86,lt:42:44 code_bytes=76,140,168,60,56 object_bytes=688,976,616,608 intra_object_calls=1 cross_object_calls=2 linked_bytes=500 output_bytes=1583 helper_result=42 persisted_reopened=1 manifest_input_bounds=2..6 malformed_build_denied=6 malformed_c_denied=21 malformed_relocation_denied=1 unresolved_symbol_denied=1 duplicate_definition_denied=1",
                )
                wait_for_output(
                ...
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_SELFHOST_LINK_OK source=guest-makfs sources=4 languages=aarch64-asm,c-subset-v1 compiler=guest-native assembler=guest-native linker=guest-native objects=4 object_format=elf64-et-rel relocations=R_AARCH64_CALL26:3 symbols=_start,answer,adjust,combine,helper build_manifest=/home/user/generated.build build_driver=makbuild-v1 build_inputs=4 cache=makstate-v2 cache_hits=0 cache_misses=4 state_committed=1 toolchain_startup=sysv manifest_arg=1 c_sources=/home/user/generated-program.c,/home/user/generated-library.c,/home/user/generated-helper.c translation_unit_functions=2,1,1 c_abi=aapcs64-int32-pointer64 c_features=multi-function,multi-parameter,six-argument,signed-arithmetic,parameter,pointer-parameter,local,array,array-decay,index,assignment,pointer,pointer-add,pointer-variable-add,pointer-difference,address-of,address-expression,dereference,if,if-assignment,if-else,nested-control,equality,inequality,relational,while,call,return max_parameters=6 max_call_arguments=6 nonleaf_frame=96,112 three_argument_result=42 three_argument_link=et-rel,same-object six_argument_result=42 six_argument_link=et-rel,same-object c_operators=mul,sdiv,srem,neg,sub,add signed_division_results=20:6,-20:-6 signed_remainder_results=20:2,-20:-2 unary_negation_results=42:-42,-42:42 arithmetic_object=elf64-et-rel:784 c_relations=eq,ne,lt,le,gt,ge branch_results=42,86 loop_results=42,2 memory_results=42,2 pointer_call=answer-to-adjust pointee_results=42,44,2 delta_results=1:42,2:44,1:2 array_results=41:42:0,42:0:44,1:2:0 pointer_offset_call=1 pointer_variable_offset=delta dynamic_pointer_adds=2 signed_pointer_offset=-1:42 signed_pointer_difference=3:-3 relational_results=gt:42:0,le:42:0,ge:42:86,lt:42:44 code_bytes=76,140,168,60,56 object_bytes=688,976,616,608 intra_object_calls=1 cross_object_calls=2 linked_bytes=500 output_bytes=1583 helper_result=42 persisted_reopened=1 manifest_input_bounds=2..6 malformed_build_denied=6 malformed_c_denied=21 malformed_relocation_denied=1 unresolved_symbol_denied=1 duplicate_definition_denied=1 output=elf64-aarch64 kernel_loader=validated abi56=1 abi57=1 argv=3 env=1 malformed_startup_denied=3 executed=2 status=42",
                )
                send_command(stream, "makbuild /home/user/generated.build")
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_MAKBUILD_OK mode=build manifest=/home/user/generated.build startup=sysv argc=2 envc=1 seeded=0 cache=makstate-v2 build_inputs=4 cache_hits=4 cache_misses=0 state_committed=1 status=42",
                )
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_MAKBUILD_CLI_OK manifest=/home/user/generated.build source=existing-makfs seeded=0 startup=sysv status=42",
                )
