    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            with client:
                ...
                wait_for_output(
                    selector, process, output,
                    b"MAKOS_AARCH64_SELFHOST_LINK_OK source=guest-makfs sources=3 languages=aarch64-asm,c-subset-v1 compiler=guest-native assembler=guest-native linker=guest-native objects=3 object_format=elf64-et-rel relocations=R_AARCH64_CALL26:2 symbols=_start,answer,adjust c_source=/home/user/generated-answer.c c_abi=aapcs64-int32 c_features=parameter,local,assignment,if,equality,inequality,while,call,return nonleaf_frame=64 c_operators=mul,sub,add branch_results=42,86 loop_results=42,2 code_bytes=76,116,108 object_bytes=688,728,664 linked_bytes=300 output_bytes=815 persisted_reopened=1 malformed_c_denied=4 malformed_relocation_denied=1 unresolved_symbol_denied=1 duplicate_definition_denied=1 output=elf64-aarch64 kernel_loader=validated abi56=1 abi57=1 argv=3 env=1 malformed_startup_denied=3 executed=2 status=42",
                )
