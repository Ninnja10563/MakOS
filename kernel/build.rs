...
fn build_aarch64_init() {
    ...
    let smp_concurrent_exit_probe_object =
        output_dir.join("aarch64-smp-concurrent-exit-probe.o");
    let smp_concurrent_exit_probe_output =
        output_dir.join("aarch64-smp-concurrent-exit-probe.elf");
    let status = Command::new("clang")
        .args([
            "-target",
            "aarch64-unknown-none-elf",
        ...
        .arg(manifest.join("../user/aarch64_smp_concurrent_exit_probe.S"))
        .arg("-o")
        .arg(&smp_concurrent_exit_probe_object)
        .status()
        .expect("failed to compile AArch64 SMP concurrent-exit userspace probe");
    assert!(
        status.success(),
        "AArch64 SMP concurrent-exit userspace probe compile failed"
    );
    let status = Command::new(rust_lld())
        .args([
            "-flavor",
        ...
        .arg(manifest.join("../user/linker-aarch64.ld"))
        .arg("-o")
        .arg(&smp_concurrent_exit_probe_output)
        .arg(&smp_concurrent_exit_probe_object)
        .status()
        .expect("failed to link AArch64 SMP concurrent-exit userspace probe");
    assert!(
        status.success(),
        "AArch64 SMP concurrent-exit userspace probe link failed"
    );

    let smp_same_group_exit_probe_object =
        output_dir.join("aarch64-smp-same-group-exit-probe.o");
    let smp_same_group_exit_probe_output =
        output_dir.join("aarch64-smp-same-group-exit-probe.elf");
    let status = Command::new("clang")
        .args([
            "-target",
            "aarch64-unknown-none-elf",
        ...
        .arg(manifest.join("../user/aarch64_smp_same_group_exit_probe.S"))
        .arg("-o")
        .arg(&smp_same_group_exit_probe_object)
        .status()
        .expect("failed to compile AArch64 SMP same-group-exit userspace probe");
    assert!(
        status.success(),
        "AArch64 SMP same-group-exit userspace probe compile failed"
    );
    let status = Command::new(rust_lld())
        .args([
            "-flavor",
        ...
        .arg(manifest.join("../user/linker-aarch64.ld"))
        .arg("-o")
        .arg(&smp_same_group_exit_probe_output)
        .arg(&smp_same_group_exit_probe_object)
        .status()
        .expect("failed to link AArch64 SMP same-group-exit userspace probe");
    assert!(
        status.success(),
    ...
    let smp_block_probe_object = output_dir.join("aarch64-smp-block-probe.o");
    let smp_block_probe_output = output_dir.join("aarch64-smp-block-probe.elf");
    let status = Command::new("clang")
        .args([
            "-target",
            "aarch64-unknown-none-elf",
        ...
        .arg(manifest.join("../user/aarch64_smp_block_probe.S"))
        .arg("-o")
        .arg(&smp_block_probe_object)
        .status()
        .expect("failed to compile AArch64 SMP block userspace probe");
    assert!(
        status.success(),
        "AArch64 SMP block userspace probe compile failed"
    );
    let status = Command::new(rust_lld())
        .args([
            "-flavor",
        ...
        .arg(manifest.join("../user/linker-aarch64.ld"))
        .arg("-o")
        .arg(&smp_block_probe_output)
        .arg(&smp_block_probe_object)
        .status()
        .expect("failed to link AArch64 SMP block userspace probe");
    assert!(
        status.success(),
